...
- La Home mostra chiaramente la versione **V.62**.
...
Corretto un problema di navigazione dal menu basso: in V.62 lo script poteva interrompersi perché cercava i pulsanti `prevMonthButton` e `nextMonthButton`, rimossi dalla Home. Ora i listener sono protetti e il menu basso cambia correttamente sezione.
...
Versione ricostruita sulla base stabile V.62.
...
- aggiunta migrazione da V.62/V.62 senza usare la logica instabile della V.62.


## Correzione V.62

Corretta la regressione della V.62:

- dichiarata correttamente la costante `APP_VERSION`;
- riallineata la chiave di salvataggio a `spese-pwa-locale-v20`;
- mantenuta la base stabile V.62/V.62;
...
- Con un solo metodo di pagamento, la maschera funziona come nella V.62: un solo campo **Metodo pagamento** e l'importo totale della spesa.
- La gestione avanzata con righe metodo/importo compare solo quando si preme **+ Aggiungi metodo**.
- Dal secondo metodo in poi resta la logica della V.62: la somma dei metodi deve coincidere con l'importo totale.
