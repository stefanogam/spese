...
- La Home mostra chiaramente la versione **V.47**.
...
Corretto un problema di navigazione dal menu basso: in V.47 lo script poteva interrompersi perché cercava i pulsanti `prevMonthButton` e `nextMonthButton`, rimossi dalla Home. Ora i listener sono protetti e il menu basso cambia correttamente sezione.
...
Versione ricostruita sulla base stabile V.47.
...
- aggiunta migrazione da V.47/V.47 senza usare la logica instabile della V.47.


## Correzione V.47

Corretta la regressione della V.47:

- dichiarata correttamente la costante `APP_VERSION`;
- riallineata la chiave di salvataggio a `spese-pwa-locale-v20`;
- mantenuta la base stabile V.47/V.47;
...
- Con un solo metodo di pagamento, la maschera funziona come nella V.45: un solo campo **Metodo pagamento** e l'importo totale della spesa.
- La gestione avanzata con righe metodo/importo compare solo quando si preme **+ Aggiungi metodo**.
- Dal secondo metodo in poi resta la logica della V.46: la somma dei metodi deve coincidere con l'importo totale.
