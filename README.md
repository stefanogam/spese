...
- La Home mostra chiaramente la versione **V.56**.
...
Corretto un problema di navigazione dal menu basso: in V.56 lo script poteva interrompersi perché cercava i pulsanti `prevMonthButton` e `nextMonthButton`, rimossi dalla Home. Ora i listener sono protetti e il menu basso cambia correttamente sezione.
...
Versione ricostruita sulla base stabile V.56.
...
- aggiunta migrazione da V.56/V.56 senza usare la logica instabile della V.56.


## Correzione V.56

Corretta la regressione della V.56:

- dichiarata correttamente la costante `APP_VERSION`;
- riallineata la chiave di salvataggio a `spese-pwa-locale-v20`;
- mantenuta la base stabile V.56/V.56;
...
- Con un solo metodo di pagamento, la maschera funziona come nella V.56: un solo campo **Metodo pagamento** e l'importo totale della spesa.
- La gestione avanzata con righe metodo/importo compare solo quando si preme **+ Aggiungi metodo**.
- Dal secondo metodo in poi resta la logica della V.56: la somma dei metodi deve coincidere con l'importo totale.
...
- Sono disponibili valori da **0 a 6** per i mesi precedenti e da **0 a 6** per i mesi successivi.
