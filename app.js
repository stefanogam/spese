...
// Elenco delle chiavi di storage usate dalle versioni precedenti (v65 -> v1),
// usato solo per la migrazione automatica di installazioni molto vecchie.
const LEGACY_STORAGE_KEYS = Array.from({ length: 65 }, (_, index) => `spese-pwa-locale-v${65 - index}`);

function findLegacySavedState() {
...
// Se il caricamento dei dati salvati fallisce (es. JSON corrotto), qui viene
// registrato cosa è successo, così l'avvio dell'app può avvisare l'utente
// invece di ripartire silenziosamente da un archivio vuoto.
...
function loadState() {
  ...
  if (!saved) {
    const oldSaved = findLegacySavedState();
    if (oldSaved) {
      try {
        const oldState = JSON.parse(oldSaved);
        return migrateState(oldState);
      } catch (error) {
        stateLoadIssue = { key: "legacy", raw: oldSaved, error };
  ...
  try {
    ...
    stateLoadIssue = { key: STORAGE_KEY, raw: saved, error };
    return structuredClone(initialState);
  }
}
...
function saveState() {
  try {
    ...
    appAlert(
      "Non sono riuscito a salvare i dati sul dispositivo (spazio esaurito o modalità privata/incognito). " +
      "Le ultime modifiche potrebbero non essere state salvate: esporta subito un backup JSON dalla sezione Soglie > Backup.",
...
if (stateLoadIssue) {
  // I dati salvati non erano leggibili: NON li sovrascriviamo subito.
  // Conserviamo il contenuto originale (illeggibile) in una chiave separata,
  // così resta un'ultima possibilità di recupero manuale, e avvisiamo l'utente.
  try {
    localStorage.setItem(`${STORAGE_KEY}-corrotto-${Date.now()}`, stateLoadIssue.raw);
  } catch (secondaryError) {
    console.error("Impossibile conservare la copia dei dati non leggibili.", secondaryError);
  }

  console.error("Dati salvati non leggibili, avvio con archivio vuoto.", stateLoadIssue.error);
  appAlert(
    "I dati salvati sul dispositivo non sono leggibili: l'app è ripartita con un archivio vuoto per poter funzionare. " +
    "Una copia (non leggibile) dei dati originali è stata conservata nel browser, per un eventuale recupero manuale. " +
    "Se hai un backup JSON o su Google Drive, usalo subito dalla sezione Soglie > Backup per ripristinare le tue spese.",
    "Dati non leggibili"
  );
} else {
  saveState();
...
function renderAll() {
  ...
  // Elementi leggeri e condivisi tra le viste (es. il menu categorie usato
  // nel form Aggiungi spesa): vengono sempre tenuti aggiornati.
  ...
  // Le sezioni più pesanti (liste, report con grafico su canvas, tabella
  // bilancio familiare) vengono ridisegnate solo se la vista è quella
  // attualmente visibile: quando l'utente cambia vista, showView() si
  // occupa già di aggiornarne il contenuto.
  const activeView = getActiveViewId();

  if (activeView === "dashboardView") renderDashboard();
  if (activeView === "expensesView") renderExpensesList();
  if (activeView === "reportView") renderReport();
  if (activeView === "familyBudgetView") renderFamilyBudget();
}
